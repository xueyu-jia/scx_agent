...
Every `commit` must include `measurement`: a low-cost read-only shell command that prints exactly one JSON object on stdout. The same measurement command is used for both A' and B'. Prefer `/proc`, `ps`, `ss`, `vmstat`, `free`, `uptime`, `cat`, `grep`, `awk`, `sed`, `cut`, `tr`, and `printf`. Do not use write redirection, network commands, background jobs, or mutating commands in measurement.
